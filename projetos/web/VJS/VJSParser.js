...
class VJSParser {
    ...
    parseVariableDeclaration(isConst = false) {
      this.advance(); // consome a keyword (variavel / const / etc)
    
      const idTok = this.expect("IDENTIFIER", "Nome da variável esperado");
      const id = { type: "Identifier", name: idTok.value };
    
      // Consome anotações de tipo opcionais: ":" TYPE|IDENTIFIER
      if (this.check("COLON") || this.check("DOIS_PONTOS")) {
        this.advance(); // consome ':' ou token equivalente
        if (this.check("TYPE") || this.check("IDENTIFIER")) {
          const typeTok = this.advance();
          // guardamos a anotação como objeto para consistência AST
          id.typeAnnotation = { type: "TypeAnnotation", name: typeTok.value };
        } else {
          throw new Error("Esperado tipo após ':' na declaração de variável");
        }
      }
